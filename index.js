  ...
  try {
    ...
    if (req.body.destination) {
      if (api.isValidAddress(req.body.destination)) {
        ...
        if (req.body.destination.startsWith("T")) {
          ...
          xAddress = addressCodec.classicAddressToXAddress(
    ...
    if (req.body.xrpAmount) {
      ...
      if (
        ...
        requestedAmountNumber > MAX_AMOUNT ||
    ...
    api
      ...
      .then((sequence) => {
        console.log(
          `${reqId}| Preparing payment with destination=${account.address}, sequence: ${sequence}`
        );
        const payment = {
          source: {
            address: address,
      ...
      .then((result) => {
        ...
        if (
          ...
          console.log(
            `${reqId}| Funded ${account.address} with ${amount} XRP (${result.engine_result})`
          );
          const response = {
            account,
          ...
          result.engine_result === "tefPAST_SEQ" ||
          result.engine_result === "terPRE_SEQ"
        ) {
          // occurs when we re-connect to a different rippled server
          //???
          console.log(
            `${reqId}| Failed to fund ${account.address} with ${amount} XRP (${result.engine_result})`
          );
          res.status(503).send({
          ...
          console.log(
            `${reqId}| Unrecognized failure to fund ${account.address} with ${amount} XRP (${result.engine_result})`
          );
          res.status(503).send({
      ...
      .catch((err) => {
        ...
        res.status(500).send({
          error:
            "Unable to fund account. Server load is too high. Try again later",
          account,
        });
        nextAvailableSeq = null;
...
function getSequenceFromAccountInfo(options) {
  ...
  return api
    ...
    .then((info) => {
      ...
      if (
        info.queue_data &&
        info.queue_data.transactions &&
        info.queue_data.transactions.length
      ) {
        ...
        for (let i = 0; i < seqs.length; i++) {
          if (sequence === seqs[i]) {
            ...
            console.log(
              `${reqId}| WARNING: found gap in Sequence: account_data.Sequence=${info.account_data.Sequence}, sequence=${sequence}, seqs[${i}]=${seqs[i]}`
            );
          } else if (sequence > seqs[i]) {
            console.log(
              `${reqId}| ERROR: invariant violated: account_data.Sequence=${info.account_data.Sequence}, sequence=${sequence}, seqs[${i}]=${seqs[i]}`
            );
          }
        }
      }
      ...
      if (!nextAvailableSeq || nextAvailableSeq === sequence) {
        ...
        console.log(
          `${reqId}| WARNING: nextAvailableSeq=${nextAvailableSeq} > sequence=${sequence}. Some prior tx likely was not applied. Setting nextAvailableSeq=${
            options.shouldAdvanceSequence ? sequence : sequence + 1
          }.`
        );
        ...
        console.log(
          `${reqId}| WARNING: nextAvailableSeq=${nextAvailableSeq} < sequence=${sequence}. Another process/server is using this funding account, or we were disconnected and reconnected to a different rippled server`
        );
        nextAvailableSeq = sequence;
        if (options.shouldAdvanceSequence === true) {
          nextAvailableSeq++;
      ...
      console.log(
        `${reqId}| called account_info; sequence: ${sequence}, account_data.Sequence=${
          info.account_data.Sequence
        }, queue_data.transactions.length=${
          info.queue_data &&
          info.queue_data.transactions &&
          info.queue_data.transactions.length
        }`
          ...
          .then((account) => {
            console.log(
              "Returning /info - ledgerVersion: " +
                info.validatedLedger.ledgerVersion +
                ", age: " +
                info.validatedLedger.age +
                ", expected_ledger_size: " +
                fee.expected_ledger_size +
                ", open_ledger_fee: " +
...
const server = app.listen(port, () =>
  console.log(
    `Altnet faucet, node version: ${process.version}, listening on port: ${port}`
  )
);
